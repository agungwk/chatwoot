  ...
  <form class="row" @submit.prevent="createChannel()">
    ...
    <div class="medium-8 columns">
      <label :class="{ error: $v.webhookURL.$error }">
        <span>
          {{ $t('INBOX_MGMT.ADD.WHATSAPP.UNOFFICIAL_WEBHOOK_URL.LABEL') }}
        </span>
        <input
          v-model.trim="webhookURL"
          type="text"
          :placeholder="
            $t('INBOX_MGMT.ADD.WHATSAPP.UNOFFICIAL_WEBHOOK_URL.PLACEHOLDER')
          "
          @blur="$v.webhookURL.$touch"
        />
        <span v-if="$v.webhookURL.$error" class="message">
          {{ $t('INBOX_MGMT.ADD.WHATSAPP.UNOFFICIAL_WEBHOOK_URL.ERROR') }}
        </span>
      </label>
    </div>
